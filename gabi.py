...
def make_dicts_for_similarity():
    context_vector = {'visitor_id': np.array([]),
                      ...
                      'age': np.array([]),
                      'referrer': np.array([])}
    offer_vector = {'header': np.array([],dtype=np.int16),
                    'language': np.array([]),
                    'adtype': np.array([]),
                    'color': np.array([]),
                    'price': np.array([])}
...
def make_new_page(p1, p2, p1pass, p1fail, p2pass, p2fail, p3, p3pass, p3fail):
    ...
    b1, b2, b3 = 0, 0, np.random.beta(p3pass, p3fail) # for only beta_page
    if p3 == BLANK:
        b3 = 0
...
def do_beta_page(ov, ov_pass, ov_fail):
    ...
    if len(ov["header"]) > STORE:
        ...
        page = {"header": ov["header"][ind],
                ...
                "price": ov["price"][ind]}
        return page
    #else return super random page
    else:
        page, suc_price = bounded_random(20, 50, "piemel")
        # page = BLANK
        return page
    pass
...
def show_us_what_you_got(from_run_id=5000, to_run_id=5010):
    ...
    # for run_id in xrange(0, 10):
        ...
        for request_number in xrange(0, req_nums):
            ...
            print("run_id=" + str(run_id) + " request_number=" + str(request_number) + " mean_reward=" + str(
                cumulative_reward / req_nums) + " success rate=" + str(num_suc) + ":" + str(
                request_number) + " price: " + str(page["price"]) + " p1-pf: " + str(p1pass) + "," + str(
                p1fail) + " p2-pf: " + str(p2pass) + "," + str(p2fail) + " p3-pf: " + str(p3pass) + "," + str(
                p3fail))

            avg_suc_price.append(num_suc)
        ...
        # ---
        avg_runid.append((run_id, cumulative_reward/req_nums, num_suc, cumulative_reward/num_suc, sum(tot_times)/req_nums))
        print("avg per runid: ")
        print(avg_runid)
